class Corner::Pos::CornerGoodPurchaseController < ApplicationController
  ...
  def create
    ...
    (1..10).each do |i|
      if params[("good_"+i.to_s).to_sym] != "empty" && params[("quantity_"+i.to_s).to_sym].to_i > 0
        ...
        good = Corner::Pos::CornerGood.find_by_number(params[("good_"+i.to_s).to_sym])  
  ...
  def get_price
    ...
    @good = Corner::Pos::CornerGood.find_by_number(params[:good])
    render json: @good
  end
end